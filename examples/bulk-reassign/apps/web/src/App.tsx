...
import { apiBase, fetchCustomers, postBulkAssign } from "./api-client";
import { BulkAssignDialog } from "./features/bulk-assign-dialog";
import { BulkResultPanel } from "./features/bulk-result-panel";
...
export function App() {
  ...
  const [url, setUrl] = useState<WorkbenchUrlState>(() => readUrlState());
  ...
  const patchUrl = useCallback((patch: Partial<WorkbenchUrlState>) => {
    setUrl((prev) => {
  ...
  return (
    <main>
      <header>
        ...
        <p className="muted">
          Example workbench · API <code>{apiBase}</code> · actor{" "}
      ...
      <SelectionBar
        ...
        onModeChange={(selectionMode) => {
          setUrl((prev) => ({ ...prev, selectionMode }));
          setPageSelected(new Set());
          setExcludedIds(new Set());
        }}
        onClear={() => {
        ...
        <CustomerTable
          ...
          selectedIds={
            url.selectionMode === "all_matching"
              ? new Set(items.map((i) => i.id).filter((id) => !excludedIds.has(id)))
              : pageSelected
          }
          selectionMode={url.selectionMode}
